...
{
    public Transform Servo1, Servo2; // Base points
    ...
    void Update()
    {
        ...
        Hinge1.position = RotateZ(H1_2D, phiRad) + Servo1.position;
        Hinge2.position = RotateZ(H2_2D, phiRad) + Servo2.position;

        // Find connection point (intersection of two circles)
        Vector3 connection = FindConnectionPoint(Hinge1.position, L1b, Hinge2.position, L2b);
        ConnectionPoint.position = connection;
